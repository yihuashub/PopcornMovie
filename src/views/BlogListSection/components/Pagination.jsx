...
import {selectArticlesOffset} from "../../../actions";
...
class PaginationSection extends React.Component {
  ...
  handleChange(newOffset) {
    this.props.dispatch(selectArticlesOffset(newOffset))
  }

  createPagination = () => {
    const maxLength = this.props.totalCount/10;
    let currentIndex = this.props.selectedArticlesOffset/10;
    let table = []
    ...
    if(currentIndex > 1)
    {
      table.unshift(
          <PaginationItem>
            <PaginationLink
                aria-label="Previous"
                onClick={() => this.handleChange(this.props.selectedArticlesOffset-2*10)}
            >
